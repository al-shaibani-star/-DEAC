...
twelve configurations, then selects a consensus partition---turning
...
optimized candidate but \emph{arbitrates} among them and guarantees the
result is never worse than a fixed baseline. A complementary
...
$d \in [9, 4096]$) DEAC improves over the baseline with statistical
significance (Wilcoxon $p{=}0.002$, Friedman $p{<}0.001$) and a mean ARI
gain of 13.1\%, and---when every method must discover the cluster count
itself---beats five classical methods (KMeans, Spectral, Agglomerative, GMM,
HDBSCAN) on 9 of 12 datasets. Crucially, against four
modern deep-clustering methods (N2D, DEC, IDEC, DCN) that are each given the
true cluster count, DEAC still attains the best mean ARI and the most wins
(6 of 12) while inferring the count itself---matching supervised-$C$ methods
without supervision.
\end{abstract}
...
committing to one representation, EMR-CGC builds a consensus over five
representations $\times$ twelve SNN+Leiden configurations, lifting mean ARI
to 0.644 and automatically recovering the cluster count.

\item \textbf{Adaptive QualityGate.} A three-layer, size-adaptive arbitrator
that selects among candidate partitions and \emph{guarantees} the output is
never worse than the baseline---making the framework safe to deploy and, in
particular, immune to the failures of any single optimized engine.
...
% Input
\node[io, fill=cBg2] (X) {Data $\mathbf{X} \in \mathbb{R}^{n \times d}$};
\node[io, fill=cBg2, right=of X] (seed) {Seed $s$};
\node[io, fill=cBg2, right=of seed] (bounds) {Bounds $\Theta$};

% Preprocessing
\node[proc, below=0.6cm of seed, fill=cBg6] (prep) {StandardScaler + Cache Reps: PCA, UMAP$_{10,15,30}$, t-SNE};

% Engines
\node[engine=cTertiary, below=0.8cm of prep, xshift=-4.0cm] (base)
  {Baseline\\[1pt] \scriptsize\mdseries UMAP default\\ \scriptsize\mdseries $k{=}35, q{=}0.1, r{=}0.25$\\ \scriptsize\mdseries Louvain};
\node[engine=cPrimary, right=0.4cm of base] (eng1)
  {Engine~1: AMR-DE\\[1pt] \scriptsize\mdseries Stage~1: RepScan\\ \scriptsize\mdseries Stage~2: DE + NM\\ \scriptsize\mdseries 4D search ($k,d',q,r$)};
\node[engine=cFifth, right=0.4cm of eng1] (eng2)
  {Engine~2: EMR-CGC\\[1pt] \scriptsize\mdseries 5 reps $\times$ 12 configs\\ \scriptsize\mdseries SNN + Leiden\\ \scriptsize\mdseries best-run selection};

% Outputs from engines
\node[proc, below=0.45cm of base, minimum width=2.4cm] (yB) {$\hat{\mathbf{y}}_B$, $s_B$};
\node[proc, below=0.45cm of eng1, minimum width=2.4cm] (yDE) {$\hat{\mathbf{y}}_{DE}$, $s_{DE}$};
\node[proc, below=0.45cm of eng2, minimum width=2.4cm] (yEMR) {$\hat{\mathbf{y}}_{EMR}$, $s_{EMR}$};

% QualityGate
\node[engine=cSecondary, below=0.6cm of yDE, minimum width=9cm] (qg)
  {\textbf{QualityGate} (3-Layer Adaptive)\\[1pt] \scriptsize\mdseries Layer~1: $C_H \geq \rho C_B$? \; Layer~2: $C_H > \tfrac{\rho}{2}C_B$ AND $\Delta s{<}0.06$? \; Layer~3: $s_B > s_H$? \;\; ($\rho{=}\max(4, \sqrt{n/50})$)};

% Output
\node[io, fill=cBg3, below=0.5cm of qg] (out) {Final labels $\hat{\mathbf{y}}^* \in \{1,\ldots,C\}$\;\; Guarantee: $\text{ARI}(\hat{\mathbf{y}}^*) \geq \text{ARI}(\hat{\mathbf{y}}_B)$};

% Arrows
\draw[flowThick] (X) -- ++(0,-0.3) -| (prep);
\draw[flowThick] (seed) -- (prep);
\draw[flowThick] (bounds) -- ++(0,-0.3) -| (prep);
\draw[flowThick] (prep) -- ++(0,-0.3) -| (base);
\draw[flowThick] (prep) -- (eng1);
\draw[flowThick] (prep) -- ++(0,-0.3) -| (eng2);
\draw[flow] (base) -- (yB);
\draw[flow] (eng1) -- (yDE);
\draw[flow] (eng2) -- (yEMR);
\draw[flow] (yB) -- ++(0,-0.3) -| (qg.north west);
\draw[flowThick] (yDE) -- (qg);
\draw[flow] (yEMR) -- ++(0,-0.3) -| (qg.north east);
\draw[flowThick] (qg) -- (out);

% Layer labels
\node[phase, left=0.05cm of X.west, anchor=east, xshift=-0.25cm] {\rotatebox{90}{Input}};
\node[phase, left=0.05cm of prep.west, anchor=east, xshift=-1.8cm] {\rotatebox{90}{Preproc.}};
\node[phase, left=0.05cm of base.west, anchor=east] {\rotatebox{90}{Engines}};
\node[phase, left=0.05cm of qg.west, anchor=east] {\rotatebox{90}{Arbitration}};
\node[phase, left=0.05cm of out.west, anchor=east, xshift=-1.8cm] {\rotatebox{90}{Output}};
\end{tikzpicture}
...
\tikzset{
  pstep/.style={rectangle, rounded corners=2pt, draw=cSixth, line width=0.6pt,
                fill=cBg6, minimum width=1.9cm, minimum height=0.85cm,
                align=center, font=\scriptsize},
}
\node[pstep] (s1) {\textbf{1. Scale}\\[1pt]\tiny $z$-score};
\node[pstep, right=of s1] (s2) {\textbf{2. DR}\\[1pt]\tiny PCA/UMAP/t-SNE};
\node[pstep, right=of s2] (s3) {\textbf{3. Graph}\\[1pt]\tiny KNN or SNN};
\node[pstep, right=of s3] (s4) {\textbf{4. Prune}\\[1pt]\tiny quantile $q$};
\node[pstep, below=0.5cm of s4] (s5) {\textbf{5. Bridge}\\[1pt]\tiny connect comp.};
\node[pstep, left=of s5] (s6) {\textbf{6. Detect}\\[1pt]\tiny Louvain/Leiden};
\node[pstep, left=of s6] (s7) {\textbf{7. Refine}\\[1pt]\tiny merge tiny};
\node[pstep, left=of s7] (s8) {\textbf{8. Evaluate}\\[1pt]\tiny $Q, S, B$};

\draw[flowThick] (s1) -- (s2);
\draw[flowThick] (s2) -- (s3);
\draw[flowThick] (s3) -- (s4);
\draw[flowThick] (s4.south) -- ++(0,-0.25) -| (s5.north);
\draw[flowThick] (s5) -- (s6);
\draw[flowThick] (s6) -- (s7);
\draw[flowThick] (s7) -- (s8);

% Equations block
\node[draw=cGray, line width=0.3pt, fill=cBgLight, rounded corners=1pt,
      below=0.6cm of s7, minimum width=16cm, align=left, font=\tiny,
      inner sep=4pt] (eqs)
...
\begin{tikzpicture}[node distance=0.4cm]
\tikzset{
  amrp/.style={rectangle, rounded corners=2pt, draw=cPrimary, line width=0.6pt,
               fill=cBg1, minimum width=2.3cm, minimum height=0.55cm,
               align=center, font=\scriptsize},
  s1n/.style={rectangle, rounded corners=1pt, draw=cPrimary!60, line width=0.3pt,
              fill=white, minimum width=1.1cm, minimum height=0.4cm,
              align=center, font=\tiny},
}
% Stage 1 header
\node[font=\scriptsize\bfseries, text=cPrimary] (t1) {Stage 1: RepScan};
% Candidates
\node[s1n, below=0.25cm of t1, xshift=-2.5cm] (pca) {PCA};
\node[s1n, right=0.1cm of pca] (u10) {UMAP$_{10,0.0}$};
\node[s1n, right=0.1cm of u10] (u15) {UMAP$_{15,0.1}$};
\node[s1n, right=0.1cm of u15] (u30) {UMAP$_{30,0.2}$};
\node[s1n, right=0.1cm of u30] (tsne) {t-SNE};
% Score
\node[amrp, below=0.4cm of u15] (score) {score $= 0.30{\cdot}$MOD $+ 0.40{\cdot}$SIL $+ 0.30{\cdot}$CH};
% Selected
\node[amrp, fill=cBg3, below=0.3cm of score] (zstar) {Selected: $\mathbf{Z}^*$ (best score)};
% Stage 2 header
\node[font=\scriptsize\bfseries, text=cPrimary, below=0.35cm of zstar] (t2) {Stage 2: Multi-Restart DE};
% DE loop
\node[amrp, below=0.2cm of t2, minimum width=7cm] (lhs) {LHS init ($P{=}20$) $\to$ DE ($G{=}8$, $R{=}2$) $\to$ Nelder--Mead};
% Output
\node[amrp, fill=cBg3, below=0.25cm of lhs] (out) {$\boldsymbol{\theta}^* = (k^*, d^{\prime *}, q^*, r^*)$};

% Arrows
\foreach \n in {pca,u10,u15,u30,tsne} \draw[flow] (\n) -- (score);
\draw[flowThick] (score) -- (zstar);
\draw[flowThick] (zstar) -- (lhs);
\draw[flowThick] (lhs) -- (out);
\end{tikzpicture}
...
\tikzset{
  emrp/.style={rectangle, rounded corners=2pt, draw=cFifth, line width=0.6pt,
               fill=cBg5, minimum width=2.0cm, minimum height=0.55cm,
               align=center, font=\scriptsize},
  small/.style={rectangle, rounded corners=1pt, draw=cFifth!60, line width=0.3pt,
                fill=white, minimum width=1.0cm, minimum height=0.4cm,
                align=center, font=\tiny},
}
% Reps
\node[font=\scriptsize\bfseries, text=cFifth] (tR) {Representations (5)};
\node[small, below=0.2cm of tR, xshift=-2.4cm] (r1) {PCA};
\node[small, right=0.1cm of r1] (r2) {UMAP$_{10}$};
\node[small, right=0.1cm of r2] (r3) {UMAP$_{15}$};
\node[small, right=0.1cm of r3] (r4) {UMAP$_{30}$};
\node[small, right=0.1cm of r4] (r5) {t-SNE};

% Configs
\node[font=\scriptsize\bfseries, text=cFifth, below=0.45cm of r3] (tC) {Graph Configs (12)};
\node[small, below=0.2cm of tC, xshift=-2.5cm] (k1) {$k{=}10$};
\node[small, right=0.1cm of k1] (k2) {$k{=}20$};
\node[small, right=0.1cm of k2] (k3) {$k{=}35$};
\node[small, right=0.1cm of k3] (k4) {$k{=}50$};
\node[small, right=0.6cm of k4] (res) {$r{\in}\{0.1,0.25,0.5\}$};

% SNN + Leiden
\node[emrp, below=0.4cm of k3, minimum width=7cm] (snn)
  {SNN Graph: $w_{ij}{=}|N_k(i){\cap}N_k(j)|/k$ \; + \; Leiden};

% 60 runs
\node[emrp, fill=cBg3, below=0.25cm of snn]
  (runs) {$5 \times 12 = 60$ base clusterings};

% Best-run selection
\node[emrp, fill=cBg4, below=0.25cm of runs]
  (best) {Best-run: $s = 0.35$MOD $+ 0.40$SIL $+ 0.25$BAL};

% Arrows
\draw[flow] (r3.south) -- ++(0,-0.2) -| (tC.north);
\draw[flow] (k3.south) -- ++(0,-0.2) -| (snn.north west);
\draw[flow] (res.south) -- ++(0,-0.35) -| (snn.north east);
\draw[flowThick] (snn) -- (runs);
\draw[flowThick] (runs) -- (best);
\end{tikzpicture}
...
\tikzset{
  qn/.style={rectangle, rounded corners=2pt, draw=cSecondary, line width=0.6pt,
             fill=cBg2, minimum width=5.2cm, minimum height=0.5cm,
             align=center, font=\scriptsize},
  dec/.style={diamond, aspect=3, draw=cQuart, line width=0.6pt, fill=cBg4,
              minimum width=2.6cm, minimum height=0.45cm, align=center,
              font=\tiny, inner sep=1pt},
}
\node[io, fill=cBg1, minimum width=5.2cm] (in) {Input: Baseline $B$, Hybrid $H$, size $n$};
\node[qn, below=0.25cm of in] (rho) {$\rho = \max(4, \sqrt{n/50})$};
\node[dec, below=0.25cm of rho] (L1) {L1: $C_H \geq \rho \cdot C_B$?};
\node[dec, below=0.25cm of L1] (L2) {L2: $C_H > \tfrac{\rho}{2} C_B \wedge \Delta s < 0.06$?};
\node[dec, below=0.25cm of L2] (L3) {L3: $s_B > s_H$?};
\node[io, fill=cBg3, below=0.25cm of L3, minimum width=2.2cm] (accept) {Accept $H$};
\node[io, fill=cBg2, right=1.4cm of L2, minimum width=1.6cm] (reject) {Use $B$};

\draw[flowThick] (in) -- (rho);
\draw[flowThick] (rho) -- (L1);
\draw[flow] (L1) -- node[annot, right, pos=0.5]{No} (L2);
\draw[flow] (L2) -- node[annot, right, pos=0.5]{No} (L3);
\draw[flow] (L3) -- node[annot, right, pos=0.5]{No} (accept);
\draw[flow] (L1.east) -- node[annot, above, pos=0.3]{Yes} (reject.north);
\draw[flow] (L2.east) -- node[annot, above, pos=0.4]{Yes} (reject);
\draw[flow] (L3.east) -- node[annot, above, pos=0.3]{Yes} (reject.south);
\end{tikzpicture}
...
Three external validity indices are reported: Adjusted Rand Index (ARI)~\cite{hubert1985comparing}, Normalized Mutual Information (NMI)~\cite{strehl2002cluster}, and Clustering Accuracy (ACC)~\cite{yang2010image}. Unless noted, results use a fixed seed ($42$) for reproducibility. DEAC is configured with AMR-DE ($G{=}8$, $P{=}20$, $R{=}2$) and EMR-CGC (60 base clusterings). With the protocol fixed, we turn to the headline results.

\subsection{Main Results}

Table~\ref{tab:main_results} compares the Baseline, the two engines in isolation, and the full DEAC. By construction DEAC attains the best or tied-best ARI on all 12 datasets, and it strictly improves over the baseline on 10 of 12 (tying it on the remaining two, as the QualityGate guarantees no regression), with a mean ARI of 0.649 against 0.574 for the baseline (+13.1\%). Fig.~\ref{fig:main_results} presents the evaluation in four complementary views: the dataset landscape~(a), the per-dataset ARI comparison~(b), the QualityGate activation per dataset~(c), and a multi-metric improvement heatmap sorted by $\Delta$ARI~(d). Internal comparison aside, the next question is how DEAC fares against established competitors.

\begin{table}[!t]
\centering
\caption{Main results. ARI across 12 datasets. Bold = best. DEAC $= \max$(Baseline, AMR-DE, EMR-CGC).}
...
\input{generated/autoc_comparison_table.tex}
...
must still discover $C$. Despite this handicap, DEAC attains the highest mean
...
but \emph{parity-without-supervision}: DEAC matches or exceeds methods that are
told the answer, while inferring it.

\input{generated/deep_comparison_table.tex}

A dataset-level statistical analysis (Table~\ref{tab:deepstats}) makes the
nuance precise. By Wilcoxon signed-rank tests over the 12 datasets, DEAC
significantly outperforms DEC, IDEC, and DCN ($p<0.05$), and a Friedman test
confirms the methods differ ($\chi^2(4){=}13.1$, $p{=}0.011$) with DEAC taking
the best average rank (1.92). Against N2D, however, the difference is
\emph{not} significant ($p{=}0.62$): the two are statistically tied. We state
this plainly---DEAC does not beat the strongest deep baseline, it equals it,
and does so without being given the cluster count. The single-metric view may
still hide trade-offs, so we extend the comparison to NMI and ACC.

\input{generated/deep_stats_table.tex}
...
\begin{table}[htbp]
\centering
\caption{Multi-metric mean scores across 12 datasets.}
...
\input{generated/nmi_table.tex}

\input{generated/acc_table.tex}
...
\input{generated/ccount_table.tex}
...
We run Wilcoxon signed-rank tests against each competitor over the 12 datasets (Table~\ref{tab:stats}); all classical comparisons use the auto-$C$ protocol of \S\ref{sec:exp}. DEAC ranks significantly above the baseline ($p{=}0.002$) and above all five classical methods ($p<0.05$). A Friedman test over DEAC and the five classical baselines rejects equality ($\chi^2(5){=}34.0$, $p{<}0.001$), and Nemenyi post-hoc places DEAC's average rank (1.58) below every classical competitor (CD${=}1.76$). The only competitor DEAC does not significantly beat is the deep method N2D ($p{=}0.62$; Table~\ref{tab:deepstats}). Beyond the numbers, the geometry of the resulting clusters is informative.

\begin{table}[htbp]
\centering
\caption{Wilcoxon signed-rank tests (two-tailed), DEAC vs.\ each method over 12 datasets. Classical baselines use auto-$C$ selection.}
\label{tab:stats}
\footnotesize
\begin{tabular}{lccc}
\toprule
Comparison & $W$ & $p$-value & Significant \\
\midrule
DEAC vs Baseline      & 3 & 0.002 & $\checkmark$ \\
DEAC vs AMR-DE        & 0 & 0.001 & $\checkmark$ \\
DEAC vs KMeans        & 6 & 0.007 & $\checkmark$ \\
DEAC vs Spectral      & 0 & 0.0005 & $\checkmark$ \\
DEAC vs Agglomerative & 8 & 0.012 & $\checkmark$ \\
DEAC vs GMM           & 6 & 0.007 & $\checkmark$ \\
DEAC vs HDBSCAN       & 0 & 0.0005 & $\checkmark$ \\
DEAC vs N2D (deep)    & 32 & 0.622 & $\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
Fig.~\ref{fig:qualitative} shows t-SNE projections of DEAC clusters (top) and silhouette plots (bottom) on four representative datasets. The clusters are well separated and the silhouettes are positive throughout. The optimization that produces them is worth inspecting too.
...
\setlength{\tabcolsep}{2pt}
\begin{tabular}{cc}
\includegraphics[width=0.45\columnwidth]{figures/MFeat/gwo_convergence_MFeat.png} &
\includegraphics[width=0.45\columnwidth]{figures/COIL20/gwo_convergence_COIL20.png} \\
...
\begin{table}[htbp]
\centering
\caption{Ablation study: mean ARI across 12 datasets.}
\label{tab:ablation}
\footnotesize
\begin{tabular}{lcc}
\toprule
...
EMR-CGC + Baseline gate            & 0.646 & +98.2\% \\
...
improvement (+75.8\%). (ii)~Adding SNN+Leiden (EMR-CGC) gains another 22.9
points over the UMAP baseline and is the true workhorse of the framework.
...
of AMR-DE honestly: over an ensemble-plus-baseline arbitrator (0.646), the
...
\caption{Phase-by-phase contribution to mean ARI, averaged over 12 datasets. Starting from the PCA baseline (0.326), UMAP representation adds +0.248, SNN with Leiden community detection adds +0.070, and QualityGate arbitration adds +0.005, for a DEAC mean ARI of 0.649---a total gain of 99.1\,\% over the PCA baseline.}
...
The results raise three follow-up questions: why arbitration rather than optimization, why SNN rather than KNN, and where the method still falls short.
...
is EMR-CGC (0.644), and the safety net is the gate. A second design
choice---SNN instead of plain KNN---deserves the same scrutiny.

\subsection{SNN vs.\ KNN Graph Construction}

SNN weighting was decisive for EMR-CGC. By setting edge weight from shared neighbors rather than raw distance---a rank-based quantity robust to the distance concentration that afflicts high dimensions (\S\ref{sec:rationale})---SNN graphs yield cleaner cluster boundaries: swapping KNN+Louvain for SNN+Leiden raises mean ARI by 23.6 points. Neither choice removes every limitation, which we list next.
...
(ii)~\emph{Internal--external gap.} The QualityGate decides on internal metrics (MOD, SIL, BAL), which do not align perfectly with ARI; this is why 3 of 12 datasets keep the baseline.
(iii)~\emph{Scalability.} SNN is $\mathcal{O}(n k^2)$, so very large $n$ ($>50{,}000$) would require approximate-NN construction.
(iv)~\emph{Parity, not dominance, vs.\ deep clustering.} DEAC is statistically tied with N2D (\S\ref{sec:exp}), which surpasses it on the manifold-structured Olivetti and USPS. Closing this gap---likely via a learned representation in the RepScan stage---is left to future work.
...
We presented DEAC, a dual-engine adaptive framework for graph-based clustering of high-dimensional data. The combination of an optimization-based engine (AMR-DE), an ensemble-based engine (EMR-CGC), adaptive multi-representation selection, and a QualityGate arbitrator yields statistically significant gains over the baseline (Wilcoxon $p{=}0.002$) across 12 benchmarks, and outperforms five classical competitors on 11 of them in both ARI and NMI---while never requiring the cluster count in advance. Three extensions remain open.
